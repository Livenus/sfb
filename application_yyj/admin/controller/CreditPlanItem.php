...
use think\Config;
class CreditPlanItem extends \app\admin\controller\Home{
    ...
    public  function listajaxAc(){
       ...
       $lists = $this->CreditPlanItem->select_all($map,"id desc",$limit);    
       foreach($lists as $k=>$v){
           $member=$this->Member->get_by_id($v['member_id']);
           $lists[$k]['uname']=$member['uname'];
           $lists[$k]['add_time']=date("Y-m-d",$v["add_time"]);
           $lists[$k]['plan_date']=date("Y-m-d H:i",$v["plan_date"]);
       }
        echo json_encode(array('rows'=>$lists,"total"=>$this->CreditPlanItem->get_count($map)));
        
    }
    public function editAc($id=0){
        $data=$this->CreditPlanItem->get_by_id($id);
        $data["member"]=$this->Member->get_by_id($data["member_id"]);
        $data["credit_plan"]=$this->CreditPlan->get_by_id($data["credit_plan_id"]);
        $data["credit"]=$this->Credit->get_by_id($data["credit_plan"]["credit_id"]);
    ...
    public function delAc(){
        $ids = input('post.')['id'];
       if(is_numeric($ids)){
           $status=$this->CreditPlanItem->del_by_id($ids);
       }else{
            $status=$this->CreditPlanItem->del_all($ids);
       }
