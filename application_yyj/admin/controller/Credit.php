...
use think\Config;
class Credit extends \app\admin\controller\Home{
    ...
    public  function listajaxAc(){
       ...
       $lists = $this->Credit->select_all($map,"id desc",$limit);    
       foreach($lists as $k=>$v){
           $member=$this->Member->get_by_id($v['member_id']);
           $lists[$k]['uname']=$member['uname'];
           $lists[$k]['add_time']=date("Y-m-d",$v["add_time"]);
       }
        echo json_encode(array('rows'=>$lists,"total"=>$this->Credit->get_count($map)));
        
    }
    public function editAc($id=0){
        $data=$this->Credit->get_by_id($id);
    ...
    public function delAc(){
        $ids = input('post.')['id'];
       if(is_numeric($ids)){
           $status=$this->Credit->del_by_id($ids);
       }else{
            $status=$this->Credit->del_all($ids);
       }
